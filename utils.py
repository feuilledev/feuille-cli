...
class Stores_Handlers():
    ...
    def dj_appstore():
        ...
        if SCRET.get_key("DJAPPSTORE") is  None or SCRET.get_key("DJAPPSTORE") == "MYKEY":
            ...
            if os.path.exists("dist/dist.tar"):
                print("[+] : Build found")
                print("[+] : Uploading...")
                ICON_PATH = f'dist/{data["assets_folder"]}/{data["icon"]}'
                x = requests.get(url="https://45.90.12.31:6517/add_app/paxo/",verify=False,cookies={"id_creator":SCRET.get_key("DJAPPSTORE")},data={"name":data["name"],"description":data["description"]},files={"icon":open(ICON_PATH,"rb"),"appfile":open("dist/dist.tar","rb")})
                print(x.status_code)
                print(x.text)
                print("[+] : Done")
                ...
                print("[TIPS] : Use feuille -l to link all files and distribute your app ! |OR use feuille -t to export to a tar")
